...
const Navbar = () => {
  ...
  return (
    <nav className="w-[100%] h-[80px] fixed flex items-center justify-between md:px-16 px-6 z-10 shadow-md shadow-gray-900">
        ...
        <Link to="https://t.me/ALTAQVOOOO" target='_blank' className='py-[10px] px-[20px] ml-4 bg-gradient-to-br from-blue-600 via-blue-400 to-blue-600 text-white rounded-md hover:scale-[98%] border-b-[3px] border-blue-800 hidden md:block'>Bog'lanish</Link>
        ...
        <div className={`${toggle ? 'block' : 'hidden'} md:hidden block w-[100%] h-[45vh] fixed top-0 left-0 bg-newBlack px-4`}>
          <ul className='py-16 px-2'>
            ...
            <Link to="/shop" className='text-white block mt-6' onClick={clickToggle}>Do'kon</Link>  
          </ul>
          <Link to="https://t.me/ALTAQVOOOO" target='_blank' className='py-[10px] px-[20px] bg-gradient-to-br from-blue-600 via-blue-400 to-blue-600 text-white rounded-md hover:opacity-85'>Bog'lanish</Link>
        </div>
    </nav>
  )
}
